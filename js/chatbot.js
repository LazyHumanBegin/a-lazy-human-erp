...
function generateChatbotFallback(message) {
    ...
    // ==================== PHASE 5.4: CHECK LEARNED RESPONSES FIRST ====================
    // 1. Apply shortcuts (e.g., "my branch" → "Branch Setapak")
    const processedMessage = applyShortcuts(message);
    const processedLower = processedMessage.toLowerCase().trim();
    
    // 2. Check if we have a learned response
    const learnedResponse = getLearnedResponse(processedLower);
    if (learnedResponse) {
        console.log(`🧠 Using learned response for: "${message}"`);
        trackFrequentQuestion(processedLower);
        return learnedResponse;
    }
    
    // 3. Track this question for learning purposes
    trackFrequentQuestion(processedLower);
    ...
    // ==================== PHASE 5.4: LEARNING COMMANDS ====================
    
    // Teach Alpha 5 custom response: "teach: how to backup = Go to Settings > Backup"
    if (lowerMessage.startsWith('teach:') || lowerMessage.startsWith('learn:')) {
        const parts = message.substring(message.indexOf(':') + 1).split('=');
        if (parts.length === 2) {
            const question = parts[0].trim();
            const answer = parts[1].trim();
            teachAlpha5(question, answer);
            return `✅ **Learned!**\n\n🎓 When you ask: _"${question}"_\n💬 I'll respond: _"${answer.substring(0, 100)}${answer.length > 100 ? '...' : ''}"_\n\n🏢 All staff in your company can now use this! 🎉`;
        } else {
            return `📚 **Teach me!**\n\nFormat: **teach: question = answer**\n\nExamples:\n• teach: how to backup = Go to Settings > Backup > Export\n• teach: refund policy = Refunds within 7 days with receipt\n• teach: delivery time = 3-5 working days\n\n💡 Once taught, all staff benefit!`;
        }
    }
    
    // Add shortcut: "shortcut: my branch = Branch Setapak"
    if (lowerMessage.startsWith('shortcut:') || lowerMessage.startsWith('alias:')) {
        const parts = message.substring(message.indexOf(':') + 1).split('=');
        if (parts.length === 2) {
            const trigger = parts[0].trim();
            const replacement = parts[1].trim();
            addShortcut(trigger, replacement);
            return `⚡ **Shortcut Created!**\n\nNow when you say _"${trigger}"_, I'll understand it as _"${replacement}"_!\n\nExample: "sales at ${trigger}" = "sales at ${replacement}"\n\n🏢 All staff can use this shortcut!`;
        } else {
            return `⚡ **Create Shortcut!**\n\nFormat: **shortcut: trigger = replacement**\n\nExamples:\n• shortcut: my branch = Branch Setapak\n• shortcut: boss = John Tan\n• shortcut: main supplier = ABC Trading\n\n💡 Speeds up repetitive typing!`;
        }
    }
    
    // Show what Alpha 5 has learned
    if (lowerMessage === 'what have you learned' || lowerMessage === 'show learning' || 
        lowerMessage === 'alpha 5 learning' || lowerMessage.includes('what did you learn')) {
        return showAlpha5Learning();
    }
    
    // Forget a learned response: "forget: how to backup"
    if (lowerMessage.startsWith('forget:') || lowerMessage.startsWith('remove:')) {
        const question = message.substring(message.indexOf(':') + 1).trim();
        if (forgetResponse(question)) {
            return `✅ Forgot the answer to: _"${question}"_\n\n💡 You can teach me again anytime!`;
        } else {
            return `🤔 I don't have a learned response for: _"${question}"_\n\nSay "show learning" to see what I know!`;
        }
    }
    
    // Export learning data for curation (Option C)
    if (lowerMessage === 'export learning' || lowerMessage === 'download learning' || 
        lowerMessage === 'export alpha 5 data') {
        exportLearningData();
        return `📥 **Learning Data Exported!**\n\nThe file includes:\n• Unanswered questions (for training)\n• Frequently asked questions\n• Custom responses\n• Shortcuts\n\n💡 Review and send to developer for global updates!`;
    }

    // ==================== DEFAULT (UNANSWERED QUESTION) ====================
    // Log this question for future training
    logUnansweredQuestion(message);
    const defaults = [
        `🤔 Hmm, I'm not sure about that yet!\n\n💡 **Teach me:** "teach: your question = the answer"\n\nOr try:\n• "show profit" / "low stock?"\n• "petrol RM50" / "sold 5 wine"\n• "go to inventory"\n• "help" for full list`,
        `I don't have an answer for that... yet! 🧠\n\n🎓 **You can train me!**\nSay: "teach: ${message} = your answer"\n\nOr ask about:\n• Financial queries\n• Inventory\n• Documents\n• Navigation`,
        `💬 I'm still learning!\n\n📝 **Train me:** Format is "teach: question = answer"\n\nMeanwhile, try:\n• "breakfast RM15" → adds expense\n• "sold 5 wine" → stock out\n• "open POS" → navigation`
    ];
